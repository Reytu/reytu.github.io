(function() {
    ...
    function checkStorage() {
        if (storageAvailable("localStorage")) {
            ...
            alert(
                "Your browser does not support the Web Storage API. Some features may not work as intended."
            );
        }
    }
    ...
    function getDelta(userDate) {
        let delta =
            Math.floor(
                (userDate - new Date().setFullYear(userDate.getFullYear())) /
                    (8.64 * Math.pow(10, 7))
            ) + 1;
        if (delta < 0) {
            delta += 365;
            ...
            for (let j = 0; j < 5; j++) {
                ...
                if (j === 0) {
                    text = document.createTextNode(
                        localStorage.getItem(keys[i]).split(",")[0]
                    );
                } else if (j === 1) {
                    var text = document.createTextNode(
                        new Date(localStorage.getItem(keys[i]).split(",")[j])
                    ...
                    text = new Image();
                    text.src =
                        "https://s3.us-east-2.amazonaws.com/upload-icon/uploads/icons/png/1460699381574330920-64.png";
                    text.alt = "Edit";
                    text.className = "edit";
                    text.id = "edit-" + i;
                    ...
                    text = new Image();
                    text.src =
                        "https://s3.us-east-2.amazonaws.com/upload-icon/uploads/icons/png/21269369161530177266-64.png";
                    text.alt = "Remove";
